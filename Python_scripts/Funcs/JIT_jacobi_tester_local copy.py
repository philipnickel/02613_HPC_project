...
# Replace this with your actual module path if needed
from jacobi import jacobi, jacobi_numba

def create_test_case(nx, ny):
    u = np.zeros((nx + 2, ny + 2))  # padded with ghost cells
    u[:, -1] = 1.0  # Dirichlet BC: right boundary = 1.0
...
def run_and_compare():
    ...
    t0 = time.perf_counter()
    u_numba_par_old = jacobi_numba(u_numba_par_old, mask, max_iter, atol, parallel=True, kind='non-fused')
    t1 = time.perf_counter()
    ...
    t0 = time.perf_counter()
    u_numba_par_new = jacobi_numba(u_numba_par_new, mask, max_iter, atol, parallel=True, kind='fused')
    t1 = time.perf_counter()
    ...
    print("L2 norm diff (numba serial):       ", np.linalg.norm(u_ref - u_numba))
    print("L2 norm diff (numba parallel old): ", np.linalg.norm(u_ref - u_numba_par_old))
    print("L2 norm diff (numba parallel new): ", np.linalg.norm(u_ref - u_numba_par_new))
    # Optional: visualize
    ...
    axs[3].imshow(u_numba_par_new, origin='lower'); axs[3].set_title("Parallel Fused")
